...
CACHE_PTH="./tmp/"
GT_CAPTION="playground/data/pancap/sapancap_val_gtcaption.json"
GT_CONTENT=${CACHE_PTH}"GT/content/sapancap_val_content.json"
GT_QUESTIONS=${CACHE_PTH}"GT/questions/sapancap_val_question.json"


############################################################################################################################################### 
# paths of cached results (usually, you may not modify this part)
PRED_DIR="playground/data/pancap_val/checkpoints/"${SAVE_CKPT}
PRED_CAPTION="playground/data/pancap_val/checkpoints/"${SAVE_CKPT}"-all-predictions.json"
PRED_CONTENT=${CACHE_PTH}${SAVE_CKPT}"/content/sapancap_val_content.json"
GT4PRED_QUESTIONS=${CACHE_PTH}${SAVE_CKPT}"/content/sapancap_val_gt4pred_question.json"
PRED_ANSWERS=${CACHE_PTH}${SAVE_CKPT}"/content/sapancap_val_pred_answer.json"
PRED_QUESTIONS=${CACHE_PTH}${SAVE_CKPT}"/content/sapancap_val_pred_question.json"
PRED4GT_QUESTIONS=${CACHE_PTH}${SAVE_CKPT}"/content/sapancap_val_pred4gt_question.json"
GT_ANSWERS=${CACHE_PTH}${SAVE_CKPT}"/content/sapancap_val_gt_answer.json"
############################################################################################################################################### 
